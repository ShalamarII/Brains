...
// src/modules/plugin-core/ObsidianTiddlyWikiPlugin.ts
var fs5 = __toESM(require("fs"));
var import_obsidian = require("obsidian");
var path5 = __toESM(require("path"));

// src/modules/format-converters/convertTiddlerContentToObsidianNoteContent.ts
function convertTiddlerContentToObsidianNoteContent(tiddlerContent) {
...
// src/modules/format-converters/parseAndNormalizeTiddlyWikiTagList.ts
function parseAndNormalizeTags(tagList) {
  function normalizeTag(tag) {
    return tag.trim().replace(/ /g, "_").normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-zA-Z0-9_]/g, "");
  }
  return tagList.replace(/\[\[(.*?)\]\]/g, (match, tag) => normalizeTag(tag));
}

// src/modules/format-converters/convertTiddlersToObsidianNotes.ts
function convertTiddlersToObsidianNotes(tiddlers) {
  var _a;
  const obsidianNotes = [];
  for (const tiddler of tiddlers) {
    const normalizedTags = parseAndNormalizeTags((_a = tiddler.tags) != null ? _a : "");
    const frontMatter = `---
${tiddler.tags ? `tags: ${normalizedTags}
` : ""}---
`;
    const content = frontMatter + convertTiddlerContentToObsidianNoteContent(tiddler.text);
    obsidianNotes.push({
      content,
      title: tiddler.title
    });
...
// src/modules/obsidian/utils/writeObsidianNotesToDirectory.ts
var fs = __toESM(require("fs"));
var path = __toESM(require("path"));
async function writeObsidianNotesToDirectory(obsidianNotes, directoryPath) {
...
// src/modules/file-manipulation/utils/convertBase64ToFileObject.ts
function convertBase64ToFileObject(base64String, fileName, mimeType) {
  const buffer = Buffer.from(base64String, "base64");
  const blob = new Blob([buffer], { type: mimeType });
  return new File([blob], fileName, { type: mimeType });
}

// src/modules/file-manipulation/utils/convertMediaFileToBase64Object.ts
var fs2 = __toESM(require("fs"));
var path2 = __toESM(require("path"));
function convertMediaFileToBase64Object(mediaFile) {
  const base64 = fs2.readFileSync(mediaFile.filePath, "base64");
  return {
    base64,
    extension: mediaFile.extension,
    mimeType: mediaFile.mimeType,
    fileName: path2.basename(mediaFile.filePath),
    creationDate: mediaFile.creationDate,
    lastModifiedDate: mediaFile.lastModifiedDate
  };
}

// src/modules/file-manipulation/utils/readFileObjectToJSON.ts
async function readFileObjectToJSON(file) {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () => {
      var _a;
      reject(new Error(`Error reading file: ${(_a = reader.error) == null ? void 0 : _a.message}`));
    };
    reader.readAsText(file);
  });
}

// src/modules/file-manipulation/utils/writeFileObjectToFilePath.ts
var import_promises = require("fs/promises");
async function writeFileObjectToFilePath(file, filePath) {
  try {
...
// src/modules/obsidian/utils/splitTagsAndTextFromObsidianNote.ts
function extractTagsFromObsidianNote(note) {
...
// src/modules/format-converters/convertObsidianNoteContentToTiddlerContent.ts
function convertObsidianNoteContentToTiddlerContent(obsidianNoteContent) {
...
${match[1]}`;
      ...
      parts[i] = parts[i].split("\n").map((line) => {
        ...
        const level = (tabs != null ? tabs : []).length + 1;
...
// src/modules/format-converters/convertObsidianNoteToTiddler.ts
function convertObsidianNoteToTiddler(obsidianNote) {
...
// src/modules/obsidian/utils/getAllObsidianNotesInDirectory.ts
var fs3 = __toESM(require("fs"));
var path3 = __toESM(require("path"));
function getAllObsidianNotesInDirectory(directoryPath) {
  ...
  for (const file of files) {
    ...
    if (fs3.statSync(filePath).isDirectory()) {
      const obsidianNotesInSubdirectory = getAllObsidianNotesInDirectory(filePath);
      obsidianNotes.push(...obsidianNotesInSubdirectory);
    } else if (path3.extname(filePath) === ".md") {
      const textNoteContent = fs3.readFileSync(filePath, "utf-8");
      const obsidianNote = {
        title: path3.basename(filePath, ".md"),
...
// src/modules/obsidian/utils/getMediaFilesInObsidianDirectory.ts
var fs4 = __toESM(require("fs"));
var path4 = __toESM(require("path"));

// src/modules/file-manipulation/utils/getFileDates.ts
...
// src/modules/file-manipulation/utils/getMimeTypeFromFilePath.ts
...
// src/modules/obsidian/utils/getMediaFilesInObsidianDirectory.ts
async function getMediaFilesInDirectory(directoryPath) {
  ...
  for (const file of files) {
    ...
    const filePath = path4.join(directoryPath, file);
    if (fs4.statSync(filePath).isDirectory()) {
      const mediaFilesInSubdirectory = await getMediaFilesInDirectory(filePath);
      mediaFiles.push(...mediaFilesInSubdirectory);
    } else if (path4.extname(filePath) !== ".md") {
...
// src/modules/tiddlywiki/utils/convertBase64ObjectToTiddler.ts
function convertBase64ObjectToTiddler(base64Object) {
  return {
...
// src/modules/plugin-core/ObsidianTiddlyWikiPlugin.ts
var ObsidianTiddlyWikiSettingsTab = class extends import_obsidian.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.handleFileUploadInputChange = async (input) => {
      if (input.files && input.files.length > 0) {
        for (let fileIndex = 0; fileIndex < input.files.length; fileIndex++) {
          ...
          fs5.mkdirSync(importPath, { recursive: true });
          const tiddlers = await readFileObjectToJSON(file);
          const nonTextTiddlers = tiddlers.filter(
            (tiddler) => {
              var _a;
              return "type" in tiddler && !((_a = tiddler.type) == null ? void 0 : _a.contains("text"));
            }
          );
          const mediaFiles = nonTextTiddlers.map(
            (nonTextTiddler) => {
              var _a;
              return convertBase64ToFileObject(
                nonTextTiddler.text,
                nonTextTiddler.title,
                (_a = nonTextTiddler.type) != null ? _a : "text/plain"
              );
            }
          );
          for (const mediaFile of mediaFiles) {
            const mediaFilePath = path5.join(importPath, mediaFile.name);
  ...
  renderExportJsonButton(pluginContainerElement) {
    pluginContainerElement.createEl("h2", { text: "Export" });
    new import_obsidian.Setting(pluginContainerElement).setName("Export JSON").setDesc(
      "The JSON file exported from this plugin can then be imported in TiddlyWiki. In TiddlyWiki go to Tools->Import"
    ).addButton(
      (button) => button.setButtonText("Export .json").onClick(async () => {
        ...
        const mediaTiddlers = mediaFiles.map(convertMediaFileToBase64Object).map(convertBase64ObjectToTiddler);
  ...
  renderImportJsonButton(pluginContainerElement) {
    ...
    new import_obsidian.Setting(pluginContainerElement).setName("Import JSON").setDesc(
  ...
  setupFileUploadInput(pluginContainerElement, form) {
  ...
  createFileInputElement(pluginContainerElement) {
  ...
  createUploadFormContainer(pluginContainerElement) {
    const form = pluginContainerElement.createEl("form", {
      attr: { encType: "multipart/form-data", hidden: true }
    });
    return form;
  }
  triggerDownloadModalForJSON(jsonObject, fileName) {
...
// src/main.ts
var ObsidianTiddlyWikiPlugin = class extends import_obsidian2.Plugin {
  async onload() {
