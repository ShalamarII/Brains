

### LEFT KEY (CAN BE TURNED)

##### BEFORE THE FOLLOWING
- Turned Any Higher Prio Left Keys
- Solved any Maze, Memory, Complicated Wires, Wire Sequence
##### AFTER THE FOLLOWING
- Right Key All
- Lower Prio Left
- Solved All Password Modules
- Solved All Who's on First Modules
- Solved All Keypad Modules

### RIGHT KEY (CAN BE TURNED)
##### BEFORE THE FOLLOWING
- Turned any Left Keys
- Turned any Lower Prio Right KEYS
- Solved any Simon Says
##### AFTER THE FOLLOWING
- Turned all the Higher Prio Right KEYS
- Solved all Morse Code, Wires, The Button, Color Flash

### ... .... . ._.. ._..

### ... .... . ._.. ._..